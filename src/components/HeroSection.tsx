...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    const tryPlay = async () => {
      ...
      try {
        video.muted = false;
        await video.play();
        // Sound autoplay succeeded (rare, but possible on desktop)
        setMuted(false);
        setLoaded(true);
        setSoundPromptVisible(true); // Ask if they want to keep sound
      } catch {
        // Browser blocked sound — try muted fallback
        video.muted = true;
        setMuted(true);
        try {
          await video.play();
          setLoaded(true);
          // Video is muted and playing — offer to enable sound
          setSoundPromptVisible(true);
        } catch {
          // Autoplay blocked entirely (very rare) — still mark loaded
          setLoaded(true);
        }
      }
    };
  ...
  // Sound was ON → user wants to mute
  const handleMute = () => {
  ...
  // Sound was ON → user wants to keep it
  const handleKeepSound = () => {
    setSoundPromptVisible(false);
    setSoundPromptDismissed(true);
  };

  // Sound was OFF (muted fallback) → user wants to enable it
  ...
  // Sound was OFF → user wants to stay muted
  ...
  return (
    ...
    >
        ...
        >
          ...
          <div style={{ display: "flex", justifyContent: "flex-end" }}>
            {soundPromptVisible && (
              ...
              >
                {muted ? (
                  // Video is muted — offer to turn sound on
                  <>
                    <p
                      style={{
                        fontFamily: YK,
                  ...
                  // Sound autoplay succeeded — ask if they want to keep it
                  <>
                    <p
                      style={{
                        fontFamily: YK,
          ...
          {/* MIDDLE — headline pinned to right */}
          <div
            style={{
              flex: 1,
              display: "flex",
          ...
          >
            <div style={{ textAlign: "right" }}>
              ...
              >
                ...
                Corner
                <br />
                <em style={{ color: "#d4a843", fontStyle: "italic" }}>Away</em>
                <br />
                From Home
              </h1>
            </div>
          ...
          {/* BOTTOM ROW — business hours left, voucher right */}
          <div
            style={{
              display: "flex",
          ...
          >
            ...
            >
              ...
              >
                Business Open
                <br />
                <span style={{ color: "#d4a843" }}>Daily from 9am – 12am</span>
              </p>
            </div>
      ...
      {/* MUTE TOGGLE — viewport-centered, outside grid */}
